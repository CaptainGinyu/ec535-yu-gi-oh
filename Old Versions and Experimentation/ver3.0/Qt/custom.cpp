...
QString activeUsr, card0, card1, card0Att, card1Att, usr0Life, usr1Life, card0Def, Mode0,card1Def,Mode1, status; // Useful strings to print on the screen
...
Custom::Custom(QWidget *parent) : QWidget(parent)
{
    ...
    imageList << "blue-eyes.png" << "dark-magician.png" << "kuriboh.png";
...
void Custom::paintEvent(QPaintEvent *event)
{
    ...
    if (gameFile.open(QIODevice::ReadOnly | QIODevice::Text)){
        ...
        messageStream >> activeUsr >> usr0Life >> card0 >> card0Att >> card0Def >> Mode0 >> usr1Life >> card1 >> card1Att >> card1Def >> Mode1 >> status;
